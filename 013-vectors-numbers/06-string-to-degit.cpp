...
int main()
{
    ...
    if (is_digit(input))
    {
        std::cout << "It's a number let's see if its valid... " << std::endl;
        if (is_valid_digit(input))
        {
            std::string output = convert_digit_to_string(input, digit_map);
            std::cout << "Your number is: " << output << std::endl;
        }
        else
        {
    ...
    else
    {
        ...
        if (out_value != -1)
        {
            std::cout << "The digit for '" << input << "' is: " << out_value << std::endl;
        }
    }
